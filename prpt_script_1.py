...
def get_species(df):
    ...
    try:
        ...
        print("Can't find the species in the input mash_dict!")


def format_species(species_str):
...
def get_ftp_file(domain, credentials_dict, target_dir, filename, output_path):
...
def get_assembly_summary(species, email, outdir=None):
    ...
    get_ftp_file(ftp_url_base, credentials_dict, species_dir, target_file, outpath)


def combine_mash_species(mash_dfs, drop_extraneous=True):
    ...
    species_dict = dict()
    for sample, df in mash_dfs.items():
        species = format_species(get_species(df))
...
def get_best_hits(species_dfs, max_hits=None):
    ...
    for species, df in species_dfs.items():
        pivoted = df.reset_index(level=0).pivot("Reference-ID", "sample", "rank")
...
def load_assembly_summaries(indir, species_list, email):
    assembly_dfs = dict()
    ...
    for species in species_list:
        ...
        if not os.path.isfile(filepath):
            get_assembly_summary(species, email, outdir=indir)
...
def get_ftp_files(url, ext, credentials_dict={}, outdir=os.getcwd()):
...
def download_assemblies(assembly_ids_dict, species_dfs, ext, n_assemblies=1):
    ...
    for species, df in assembly_ids_dict.items():
        ...
        ftp_paths.apply(get_ftp_files, ext=ext, outdir=ref_dir)
...
def main():
    ...
    mash_dfs = get_mash_dfs(os.path.join(OUTDIR, "mash_distances"))
    write_sample_species(mash_dfs)
    species_dfs = combine_mash_species(mash_dfs)
    best_hits = get_best_hits(species_dfs)
    for species in best_hits:
        get_assembly_summary(species, EMAIL, outdir=OUTDIR)
    assembly_dfs = load_assembly_summaries(OUTDIR, list(best_hits.keys()), EMAIL)
    assembly_dict = assemb_ids_from_mash(species_dfs, assembly_dfs)
    exts = ["fna", "gff"]
    if PHYLO == "lyveset":
        exts.append("gbff")
    for ext in exts:
        sample_build_dict = download_assemblies(assembly_dict, species_dfs, ext=ext)
